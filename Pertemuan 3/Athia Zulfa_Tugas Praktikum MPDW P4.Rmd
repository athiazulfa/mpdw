...
# Baca file CSV
data <- read.csv("C:/Users/Athia Zulfa/Downloads/DATA MPDW P3.csv", header = TRUE, sep = ";")
...
Berdasarkan penelitian High-resolution modeling for criteria air pollutants in Shanghai, ditemukan bahwa O₃, PM₂.₅, dan NO₂ merupakan tiga polutan utama yang secara signifikan menentukan variasi kualitas udara di wilayah perkotaan. Hasil model resolusi tinggi dalam studi tersebut memperlihatkan bahwa kontribusi NO₂ tidak hanya penting sebagai polutan langsung, tetapi juga sebagai prekursor dalam pembentukan ozon troposferik. Hal ini menegaskan bahwa NO₂ memiliki peran ganda dalam memengaruhi indeks kualitas udara (AQI).
...
-   Koefisien $Y_{t−1}$ (<2e-16) juga signifikan. Ini adalah **estimasi untuk** $\lambda$. Nilai ini menunjukkan danya autokorelasi  antara nilai Y dari satu periode ke periode berikutnya.
...
Berdasarkan plot tersebut, terlihat bahwa plot yang paling mendekati data aktualnya adalah Model Distributed Lag dan Autoregressive, namun berdasarkan nilai MAPE, Model Distributed Lag memiliki nilai MAPE yang lebih kecil. Sehingga dapat disimpulkan model terbaik dalam hal ini adalah model Distributed Lag.
